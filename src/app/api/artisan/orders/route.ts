...
async function sendShippedEmail(order: any, trackingUrl: string | undefined, shortId: string) {
  ...
  await resend.emails.send({
    ...
    to: order.user_email,
    subject: `Your order #${shortId} has shipped — KalaStree`,
    html: `
      <div style="font-family:Georgia,serif;max-width:560px;margin:0 auto;padding:32px;background:#FFF8EE;border:1.5px solid #DDB840;border-radius:10px;">
        ...
        <p style="color:#6B4820;line-height:1.8;margin-bottom:24px;">
          Your order <strong>#${shortId}</strong> has been shipped and is on its way to you.
        </p>
        ${trackingUrl ? `
        <div style="text-align:center;margin-bottom:24px;">
          <a href="${trackingUrl}" style="display:inline-block;background:#1B2E4A;color:#D4A000;font-weight:700;padding:14px 28px;border-radius:8px;text-decoration:none;">Track Your Order</a>
        </div>` : ''}
...
export async function PATCH(req: NextRequest) {
  ...
  if (status === 'shipped') {
    ...
    // Send shipped notification (non-blocking) — delivery confirmation itself
    // is left entirely to the iThink courier webhook (see /api/webhooks/ithink).
    try {
      const { data: order } = await supabaseAdmin
        .from('orders').select('user_name, user_email').eq('id', orderId).single()
      if (order) await sendShippedEmail(order, shipment.trackingUrl, orderId.slice(0, 8).toUpperCase())
    } catch (e) {
