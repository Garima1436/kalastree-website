...
const CATEGORY_COLORS: Record<string, string> = {
  textile: '#16303A', handicraft: '#1E5F74', agricultural: '#1A7A32', food: '#C21859',
}
...
function CardVisual({ product }: { product: GIProduct }) {
  ...
  return (
    <div style={{ height: 200, position: 'relative', borderRadius: '10px 10px 0 0', overflow: 'hidden', background: `linear-gradient(135deg, ${product.accent}18, ${product.accent}30)` }}>
      ...
      <div style={{ position: 'absolute', top: 10, right: 10, background: '#1E5F74', color: '#fff', fontSize: '0.6rem', fontWeight: 700, padding: '3px 8px', borderRadius: 4, fontFamily: "'Inter', sans-serif" }}>
        {t('giCertified')}
      </div>
    </div>
  )
...
function ProductModal({ product, onClose }: { product: GIProduct; onClose: () => void }) {
  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 1000, background: 'rgba(10,5,0,0.65)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
      <div onClick={e => e.stopPropagation()} style={{ background: '#F5F7F6', borderRadius: 16, maxWidth: 720, width: '100%', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 24px 80px rgba(0,0,0,0.3)', border: '1.5px solid #C7D2D6' }}>
        {/* Header */}
        <div style={{ height: 200, background: `linear-gradient(135deg, ${product.accent}25, ${product.accent}45)`, borderRadius: '14px 14px 0 0', display: 'flex', alignItems: 'flex-end', gap: '1.5rem', padding: '0 2rem 1.25rem', position: 'relative', overflow: 'hidden' }}>
          ...
          <button onClick={onClose} style={{ position: 'absolute', top: 14, right: 14, zIndex: 2, background: 'rgba(255,255,255,0.85)', border: 'none', borderRadius: '50%', width: 34, height: 34, cursor: 'pointer', fontSize: '1.1rem', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#5B7480', fontWeight: 700 }}>✕</button>
        </div>

        <div style={{ padding: '2rem' }}>
          {/* Women involvement */}
          <div style={{ background: 'linear-gradient(135deg, #F5F7F6, #F5F7F6)', border: `1.5px solid ${product.accent}40`, borderLeft: `4px solid ${product.accent}`, borderRadius: 10, padding: '1.25rem 1.5rem', marginBottom: '1.5rem' }}>
          ...
          <div style={{ marginBottom: '1.5rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: '0.75rem' }}>
              ...
              <span style={{ fontFamily: "'Inter', sans-serif", fontSize: '0.75rem', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#5B7480' }}>{t('historyAndHeritage')}</span>
            </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
            <div style={{ background: '#EEF2F2', borderRadius: 8, padding: '1rem' }}>
              <div style={{ fontFamily: "'Inter', sans-serif", fontSize: '0.7rem', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#7C93A0', marginBottom: 6 }}>{t('materials')}</div>
              <p style={{ fontFamily: "'Inter', sans-serif", fontSize: '0.88rem', color: '#3A1C08', lineHeight: 1.6, margin: 0 }}>{materials}</p>
            </div>
            <div style={{ background: '#EEF2F2', borderRadius: 8, padding: '1rem' }}>
              <div style={{ fontFamily: "'Inter', sans-serif", fontSize: '0.7rem', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#7C93A0', marginBottom: 6 }}>{t('districts')}</div>
              <p style={{ fontFamily: "'Inter', sans-serif", fontSize: '0.88rem', color: '#3A1C08', lineHeight: 1.6, margin: 0 }}>{product.district}</p>
            </div>
          </div>

          {/* Footer */}
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '1.5rem', paddingTop: '1.25rem', borderTop: '1px solid #DCE4E6' }}>
            <div style={{ fontFamily: "'Inter', sans-serif", fontSize: '0.8rem', color: '#7C93A0' }}>{product.gi_tag} · {t('certified')} {product.year}</div>
            <Link href={`/shop?state=${encodeURIComponent(product.state)}`} style={{ background: '#1E5F74', color: '#fff', fontFamily: "'Inter', sans-serif", fontSize: '0.78rem', fontWeight: 700, letterSpacing: '0.07em', textTransform: 'uppercase', padding: '8px 18px', borderRadius: 6, textDecoration: 'none' }}>
...
export default function GIProductsClient({ products }: { products: GIProduct[] }) {
  ...
  return (
    <div style={{ background: 'var(--parchment)', minHeight: '100vh' }}>
      {/* Hero */}
      <div style={{ background: 'linear-gradient(160deg, #16303A 0%, #0D1E33 100%)', padding: '4rem 5% 5rem', position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', right: '5%', top: '50%', transform: 'translateY(-50%)', opacity: 0.06, pointerEvents: 'none' }}>
          <svg width="420" height="420" viewBox="0 0 420 420" fill="none">
            <circle cx="210" cy="210" r="200" stroke="#1E5F74" strokeWidth="1" />
            <circle cx="210" cy="210" r="160" stroke="#1E5F74" strokeWidth="1" />
            <circle cx="210" cy="210" r="120" stroke="#1E5F74" strokeWidth="1" />
            <circle cx="210" cy="210" r="80" stroke="#1E5F74" strokeWidth="1" />
            {[0,30,60,90,120,150,180,210,240,270,300,330].map(angle => (
              <line key={angle} x1="210" y1="10" x2="210" y2="210" stroke="#1E5F74" strokeWidth="0.5" transform={`rotate(${angle} 210 210)`} />
            ))}
          </svg>
        </div>
        <div style={{ maxWidth: 900, margin: '0 auto', position: 'relative', zIndex: 1 }}>
          <p style={{ fontFamily: "'Inter', sans-serif", fontSize: '0.7rem', fontWeight: 700, letterSpacing: '0.25em', textTransform: 'uppercase', color: '#1E5F74', marginBottom: '0.75rem' }}>
            <Link href="/" style={{ color: '#1E5F74', textDecoration: 'none' }}>{tc('home')}</Link>
          ...
          <h1 style={{ fontFamily: "'EB Garamond', serif", fontSize: 'clamp(2.4rem, 5vw, 3.8rem)', fontWeight: 700, color: '#fff', lineHeight: 1.15, marginBottom: '1rem' }}>
            {t('heroTitlePrefix')}<span style={{ color: '#1E5F74', fontStyle: 'italic' }}>{t('heroTitleAccent')}</span>
          </h1>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '2rem' }}>
            {[{ n: '478', l: t('statGiTags') }, { n: String(products.length), l: t('statProductsProfiled') }, { n: '16', l: t('statStatesCovered') }, { n: '2,500+', l: t('statWomenVoices') }].map(({ n, l }) => (
              <div key={l}>
                <div style={{ fontFamily: "'EB Garamond', serif", fontSize: '2rem', fontWeight: 700, color: '#1E5F74', lineHeight: 1 }}>{n}</div>
      ...
      {/* Controls */}
      <div style={{ background: '#F5F7F6', borderBottom: '1.5px solid #C7D2D6', position: 'sticky', top: 64, zIndex: 50 }}>
        <div style={{ maxWidth: 1300, margin: '0 auto', padding: '0 4%' }}>
          <div style={{ overflowX: 'auto', display: 'flex', gap: '0.15rem', padding: '0.75rem 0', scrollbarWidth: 'none' }}>
            {allStates.map(state => {
              ...
              return (
                <button key={state} onClick={() => setActiveState(state)} style={{ flexShrink: 0, fontFamily: "'Inter', sans-serif", fontSize: '0.78rem', fontWeight: 700, letterSpacing: '0.05em', padding: '7px 14px', borderRadius: 6, border: active ? '1.5px solid #1E5F74' : '1.5px solid transparent', background: active ? '#1E5F74' : 'transparent', color: active ? '#fff' : '#5B7480', cursor: 'pointer', transition: 'all 0.15s', whiteSpace: 'nowrap' }}>
                  {state === 'All States' ? t('allStatesLabel') : state}
                  <span style={{ marginLeft: 6, background: active ? 'rgba(255,255,255,0.25)' : '#DCE4E6', color: active ? '#fff' : '#9B6820', borderRadius: 10, padding: '1px 7px', fontSize: '0.65rem', fontWeight: 700 }}>
                    {stateCount(state)}
                  </span>
                </button>
              )
          ...
          <div style={{ padding: '0.6rem 0', borderTop: '1px solid #DCE4E6' }}>
            <input type="text" placeholder={t('searchPlaceholder')} value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
              style={{ width: '100%', maxWidth: 480, fontFamily: "'Inter', sans-serif", fontSize: '0.88rem', padding: '8px 14px', border: '1.5px solid #C7D2D6', borderRadius: 8, background: '#F5F7F6', color: '#16303A', outline: 'none', boxSizing: 'border-box' }} />
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 1300, margin: '0 auto', padding: '2.5rem 4%' }}>
        <div style={{ marginBottom: '1.5rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '0.75rem' }}>
          <p style={{ fontFamily: "'Inter', sans-serif", fontSize: '0.85rem', color: '#9B6820', margin: 0 }}>
            {t('showing')} <strong style={{ color: '#16303A' }}>{filtered.length}</strong> {t(filtered.length !== 1 ? 'giCertifiedProducts' : 'giCertifiedProduct')}
            {activeState !== 'All States' && <> {t('from')} <strong style={{ color: '#1E5F74' }}>{activeState}</strong></>}
          </p>
        ...
        {filtered.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '5rem 0' }}>
            ...
            <p style={{ fontFamily: "'EB Garamond', serif", fontSize: '1.4rem', color: '#5B7480' }}>{t('noProductsFoundTitle')}</p>
            <button onClick={() => { setSearchQuery(''); setActiveState('All States') }} style={{ marginTop: '1rem', background: '#1E5F74', color: '#fff', border: 'none', borderRadius: 6, padding: '8px 20px', cursor: 'pointer', fontFamily: "'Inter', sans-serif", fontWeight: 700 }}>{t('clearFilters')}</button>
          </div>
        ) : (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '1.5rem' }}>
            {filtered.map(product => (
              <div key={product.id} onClick={() => setSelectedProduct(product)}
                style={{ background: '#F5F7F6', border: '1.5px solid #C7D2D6', borderRadius: 12, overflow: 'hidden', cursor: 'pointer', transition: 'transform 0.18s, box-shadow 0.18s', boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}
              ...
              >
                ...
                <div style={{ padding: '1.25rem' }}>
                  <div style={{ fontFamily: "'Inter', sans-serif", fontSize: '0.68rem', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#7C93A0', marginBottom: '0.35rem' }}>{product.state}</div>
                  <h3 style={{ fontFamily: "'EB Garamond', serif", fontSize: '1.2rem', fontWeight: 700, color: '#16303A', margin: '0 0 0.5rem' }}>{localizedGiField(product.name, product.name_hi, lang)}</h3>
                  <p style={{ fontFamily: "'Inter', sans-serif", fontSize: '0.82rem', color: '#5B7480', lineHeight: 1.6, margin: '0 0 1rem' }}>{localizedGiField(product.tagline, product.tagline_hi, lang)}</p>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: '0.75rem', borderTop: '1px solid #DCE4E6' }}>
                    <span style={{ fontFamily: "'Inter', sans-serif", fontSize: '0.68rem', color: '#1E5F74', fontWeight: 700 }}>{product.gi_tag}</span>
                    <span style={{ fontFamily: "'Inter', sans-serif", fontSize: '0.72rem', fontWeight: 700, color: '#1E5F74', letterSpacing: '0.05em' }}>{t('learnMore')}</span>
                  </div>
                </div>
              </div>
            ))}
      ...
      {/* Knowledge Banner */}
      <div style={{ background: '#16303A', padding: '4rem 5%', marginTop: '2rem' }}>
        <div style={{ maxWidth: 1000, margin: '0 auto', textAlign: 'center' }}>
          <p style={{ fontFamily: "'Inter', sans-serif", fontSize: '0.72rem', fontWeight: 700, letterSpacing: '0.2em', textTransform: 'uppercase', color: '#1E5F74', marginBottom: '1rem' }}>{t('aiPoweredKnowledge')}</p>
          ...
          <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center', flexWrap: 'wrap' }}>
            <Link href="/chatbot" style={{ fontFamily: "'Inter', sans-serif", fontSize: '0.85rem', fontWeight: 700, letterSpacing: '0.07em', textTransform: 'uppercase', background: '#1E5F74', color: '#fff', padding: '12px 28px', borderRadius: 6, textDecoration: 'none' }}>{t('chatWithAi')}</Link>
            <Link href="/shop" style={{ fontFamily: "'Inter', sans-serif", fontSize: '0.85rem', fontWeight: 700, letterSpacing: '0.07em', textTransform: 'uppercase', background: 'transparent', color: '#1E5F74', border: '1.5px solid #1E5F74', padding: '12px 28px', borderRadius: 6, textDecoration: 'none' }}>{t('shopGiProducts')}</Link>
          </div>
        </div>
      </div>
      ...
      <style>{`
        input::placeholder { color: #7C93A0; }
        input:focus { border-color: #1E5F74 !important; box-shadow: 0 0 0 3px rgba(232,56,10,0.12); }
