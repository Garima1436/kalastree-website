...
export default function ChatbotPage() {
  ...
  return (
    <div style={{ background: 'var(--parchment)', minHeight: '80vh' }}>
      {/* Header */}
      <div style={{ background: '#DCE4E6', padding: '3.5rem 5%', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth: 760, margin: '0 auto', textAlign: 'center', position: 'relative', zIndex: 1 }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: '#16303A', color: '#1E5F74', padding: '6px 16px', borderRadius: 30, fontSize: '0.72rem', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: '1.2rem' }}>
          ...
          <h1 style={{ fontFamily: "'EB Garamond', serif", fontSize: 'clamp(2rem, 4vw, 3rem)', fontWeight: 700, color: '#16303A', marginBottom: '1rem' }}>
            KalaStree <span style={{ color: '#1E5F74' }}>{t('giResearchHighlight')}</span> {t('aiChatbotSuffix')}
          </h1>
          <p style={{ color: '#5B7480', fontSize: '1rem', lineHeight: 1.8 }}>
            {t('heroDescription')}
          </p>
          <p style={{ marginTop: '0.75rem', fontSize: '0.85rem', color: '#5B7480' }}>
      ...
      <div style={{ maxWidth: 800, margin: '0 auto', padding: '3rem 5%' }}>
        <div style={{ background: '#16303A', borderRadius: 12, padding: '1.5rem', marginBottom: '2rem' }}>
          {/* Chat header */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: '1.25rem', paddingBottom: '1rem', borderBottom: '1px solid rgba(212,160,0,0.2)' }}>
            <div style={{ width: 36, height: 36, borderRadius: '50%', background: 'linear-gradient(135deg, #1E5F74, #1E5F74)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1rem' }}>🌾</div>
            <div>
              ...
              {history.map((turn, i) => (
                <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: turn.role === 'user' ? 'flex-end' : 'flex-start' }}>
                  <div style={{
                    ...
                    background: turn.role === 'user' ? '#1E5F74' : 'rgba(255,255,255,0.08)',
                  ...
                  {turn.sources && turn.sources.length > 0 && (
                    <div style={{ fontSize: '0.72rem', color: 'rgba(255,255,255,0.4)', marginTop: 3, paddingLeft: 4 }}>
                      <strong style={{ color: '#1E5F74' }}>{t('sourcesLabel')}</strong> {turn.sources.join(', ')}
                    </div>
                  )}
                </div>
              ))}
          ...
          {loading && (
            <div style={{ display: 'flex', gap: 4, padding: '10px 14px', alignItems: 'center', marginBottom: '1rem' }}>
              {[0, 1, 2].map(i => (
                <span key={i} style={{ width: 6, height: 6, borderRadius: '50%', background: '#1E5F74', display: 'inline-block', animation: `bounce 1.4s ease-in-out ${i * 0.2}s infinite` }} />
              ))}
            </div>
          )}
          ...
          <button
            ...
            style={{ width: '100%', padding: '12px', background: loading ? '#555' : '#1E5F74', color: '#fff', border: 'none', borderRadius: 8, fontWeight: 700, fontSize: '0.95rem', cursor: loading ? 'not-allowed' : 'pointer', transition: 'background 0.2s' }}
          >
        ...
        {history.length === 0 && (
          <div>
            <p style={{ fontSize: '0.75rem', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#5B7480', marginBottom: '1rem' }}>{t('tryAsking')}</p>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              {SAMPLE_QUESTIONS.map(q => (
                <button
                  key={q}
                  onClick={() => ask(q)}
                  style={{ background: '#E3ECEE', border: '1px solid #C7D2D6', borderRadius: 20, padding: '6px 14px', fontSize: '0.8rem', color: '#5B7480', cursor: 'pointer', fontFamily: "'Inter', sans-serif", textAlign: 'left' }}
                >
                  {q}
                </button>
              ))}
