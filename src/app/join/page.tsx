...
export default function JoinPage() {
  ...
  const inp = { width: '100%', background: 'rgba(253,246,227,0.8)', border: '1.5px solid #C7D2D6', borderRadius: 6, padding: '12px 16px', fontFamily: "'Inter', sans-serif", fontSize: '0.95rem', color: '#1A1A1A', outline: 'none' }
      ...
      <div style={{ background: '#1A7A32', padding: '3.5rem 5%' }}>
        <div style={{ maxWidth: 800, margin: '0 auto' }}>
          <p style={{ fontSize: '0.72rem', fontWeight: 700, letterSpacing: '0.2em', textTransform: 'uppercase', color: '#1E5F74', marginBottom: 8 }}>
            <Link href="/" style={{ color: '#1E5F74', textDecoration: 'none' }}>{tc('home')}</Link> / {t('breadcrumbSell')}
          </p>
          <h1 style={{ fontFamily: "'EB Garamond', serif", fontSize: 'clamp(2rem, 4vw, 3rem)', fontWeight: 700, color: '#fff', marginBottom: 12 }}>
            {t('heroTitlePart1')} <span style={{ color: '#1E5F74' }}>{t('heroTitlePart2')}</span>
          </h1>
      ...
      <div style={{ background: '#C8F5D8', padding: '2.5rem 5%' }}>
        <div style={{ maxWidth: 800, margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '1.5rem' }}>
          {[
            ...
            <div key={title} style={{ background: '#F5F7F6', borderRadius: 10, padding: '1.25rem', border: '1.5px solid #C7D2D6', textAlign: 'center' }}>
              <div style={{ fontSize: '1.8rem', marginBottom: 8 }}>{icon}</div>
              <div style={{ fontFamily: "'EB Garamond', serif", fontSize: '1rem', fontWeight: 600, color: '#1A7A32', marginBottom: 4 }}>{title}</div>
              <div style={{ fontSize: '0.8rem', color: '#5B7480', lineHeight: 1.5 }}>{body}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ maxWidth: 800, margin: '0 auto', padding: '4rem 5%' }}>
        {status === 'success' ? (
          <div style={{ textAlign: 'center', padding: '4rem', background: '#C8F5D8', borderRadius: 12, border: '2px solid #1A7A32' }}>
            ...
            <p style={{ color: '#5B7480', lineHeight: 1.8, marginBottom: '1.5rem' }}>
              {t('successThankYouPrefix')} {form.name}. {t('successBodyMiddle')} <strong>{form.email}</strong>.
            </p>
            <Link href="/shop" style={{ background: '#1E5F74', color: '#fff', padding: '12px 24px', borderRadius: 5, fontWeight: 700, textDecoration: 'none' }}>
              {t('successCta')}
            </Link>
          </div>
        ) : (
          <form onSubmit={handleSubmit} style={{ background: '#F5F7F6', border: '1.5px solid #C7D2D6', borderRadius: 12, padding: '2.5rem' }}>
            <h2 style={{ fontFamily: "'EB Garamond', serif", fontSize: '1.8rem', fontWeight: 600, color: '#16303A', marginBottom: '2rem' }}>{t('formTitle')}</h2>

            <div className="join-form-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
              <div>
                <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: '#5B7480', marginBottom: 8 }}>{t('labelName')}</label>
                <input required style={inp} value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="Meena Devi" />
              </div>
              <div>
                <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: '#5B7480', marginBottom: 8 }}>{t('labelEmail')}</label>
            ...
            <div className="join-form-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
              <div>
                <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: '#5B7480', marginBottom: 8 }}>{t('labelPhone')}</label>
                <input style={inp} value={form.phone} onChange={e => setForm({ ...form, phone: e.target.value })} placeholder="+91 98765 43210" />
              </div>
              <div>
                <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: '#5B7480', marginBottom: 8 }}>{t('labelState')}</label>
            ...
            <div style={{ marginBottom: '1rem' }}>
              <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: '#5B7480', marginBottom: 8 }}>{t('labelCraft')}</label>
              <input required style={inp} value={form.craft} onChange={e => setForm({ ...form, craft: e.target.value })} placeholder={t('placeholderCraft')} />
            </div>

            <div style={{ marginBottom: '1rem' }}>
              <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: '#5B7480', marginBottom: 8 }}>{t('labelGiProduct')}</label>
            ...
            <div style={{ marginBottom: '1.5rem' }}>
              <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: '#5B7480', marginBottom: 8 }}>{t('labelMessage')}</label>
            ...
            {status === 'error' && <p style={{ color: '#1E5F74', textAlign: 'center', marginTop: '1rem', fontSize: '0.9rem' }}>{t('errorMessage')}</p>}
          </form>
        )}
        <style>{`
