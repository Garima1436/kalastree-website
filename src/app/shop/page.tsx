  ...
  return (
    <div style={{ minHeight: '80vh', background: 'var(--parchment)' }}>
      ...
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '3rem 5%', display: 'grid', gridTemplateColumns: '220px 1fr', gap: '3rem', alignItems: 'start' }}>
        {/* Sidebar Filters */}
        <aside>
          <div style={{ background: '#FFFEF9', border: '1.5px solid #D9C9A8', borderRadius: 10, padding: '1.5rem', position: 'sticky', top: 90 }}>
            <div style={{ fontWeight: 700, fontSize: '0.78rem', letterSpacing: '0.1em', textTransform: 'uppercase', color: '#5C5542', marginBottom: '1rem' }}>Categories</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              <Link href="/shop" style={{ fontSize: '0.9rem', fontWeight: !activeCategory ? 700 : 400, color: !activeCategory ? '#C94B1A' : '#5C5542', textDecoration: 'none', padding: '6px 10px', borderRadius: 6, background: !activeCategory ? '#F5E8E0' : 'transparent' }}>
                🌟 All Products
              </Link>
              {(Object.entries(CATEGORY_META) as [Category, typeof CATEGORY_META[Category]][]).map(([key, meta]) => (
                <Link key={key} href={`/shop?category=${key}`} style={{ fontSize: '0.9rem', fontWeight: activeCategory === key ? 700 : 400, color: activeCategory === key ? meta.color : '#5C5542', textDecoration: 'none', padding: '6px 10px', borderRadius: 6, background: activeCategory === key ? meta.bg : 'transparent' }}>
                  {meta.icon} {meta.label}
                </Link>
              ))}
            </div>

            <div style={{ borderTop: '1px solid #D9C9A8', marginTop: '1.5rem', paddingTop: '1.5rem' }}>
              <div style={{ fontWeight: 700, fontSize: '0.78rem', letterSpacing: '0.1em', textTransform: 'uppercase', color: '#5C5542', marginBottom: '1rem' }}>Filter by State</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {['Bihar', 'Jammu & Kashmir', 'West Bengal', 'Uttar Pradesh', 'Rajasthan', 'Tamil Nadu', 'Assam', 'Punjab'].map(state => (
                  <Link key={state} href={`/shop?state=${encodeURIComponent(state)}`} style={{ fontSize: '0.85rem', color: params.state === state ? '#C94B1A' : '#5C5542', textDecoration: 'none', fontWeight: params.state === state ? 700 : 400 }}>
                    {state}
                  </Link>
                ))}
              </div>
        ...
        <div>
          {products.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '5rem 0', color: '#5C5542' }}>
              ...
              <Link href="/join" style={{ color: '#C94B1A', fontWeight: 700 }}>Know an artisan? Invite them to join →</Link>
            </div>
          ) : (
