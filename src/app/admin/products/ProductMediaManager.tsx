...
export default function ProductMediaManager({ productId }: { productId: string }) {
  ...
  const uploadImages = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      if (file.size > 10 * 1024 * 1024) { setError(t('fileExceeds10MB').replace('{file}', file.name)); continue }
      ...
      if (upErr) { setError(t('uploadFailed').replace('{message}', upErr.message)); continue }
  ...
  const uploadVideo = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return
    if (!file.type.startsWith('video/')) { setError(t('selectVideoFile')); return }
    if (file.size > 200 * 1024 * 1024) { setError(t('videoUnder200MB')); return }
    ...
    const { error: upErr } = await supabase.storage.from('products').upload(fileName, file, { contentType: file.type, upsert: true })
    if (upErr) { setError(t('videoUploadFailed').replace('{message}', upErr.message)); setUploadingVideo(false); return }
  ...
  const addYouTube = async () => {
    ...
    if (!id) { setYtError(t('invalidYoutubeUrl')); return }
  ...
  const deleteMedia = async (item: MediaItem) => {
    const typeLabel = item.type === 'video' ? t('mediaTypeVideo') : t('mediaTypeImage')
    if (!confirm(t('deleteMediaConfirm').replace('{type}', typeLabel))) return
  ...
  return (
    <div style={{ marginTop: '2rem', borderTop: '2px solid #DDB840', paddingTop: '1.75rem' }}>
            ...
            <div key={item.id} style={{ position: 'relative', borderRadius: 8, overflow: 'hidden', border: '1.5px solid #DDB840', background: '#FFE8A8', aspectRatio: '4/3' }}>
              ...
              <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top, rgba(0,0,0,0.7) 0%, transparent 50%)', display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', padding: '6px' }}>
                <div style={{ display: 'flex', gap: 3 }}>
                  <button onClick={() => move(idx, -1)} disabled={idx === 0} title={t('moveLeftTitle')}
                  ...
                  <button onClick={() => move(idx, 1)} disabled={idx === media.length - 1} title={t('moveRightTitle')}
                ...
                <button onClick={() => deleteMedia(item)} title={t('deleteTitle')}
              ...
              {idx === 0 && <div style={{ position: 'absolute', top: 5, right: 5, background: '#D4A000', color: '#fff', fontSize: '0.5rem', fontWeight: 700, padding: '2px 5px', borderRadius: 3 }}>{t('coverBadge')}</div>}
            </div>
          ))}
        </div>
      )}
      ...
      {media.length === 0 && (
        <p style={{ color: '#A07840', fontSize: '0.85rem', marginBottom: '1.25rem', fontStyle: 'italic' }}>{t('noMediaYet')}</p>
      )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.25rem' }}>
        ...
        <div style={{ background: '#FFF5E0', border: '1.5px solid #DDB840', borderRadius: 8, padding: '1.25rem' }}>
          <label style={labelStyle}>📷 {t('addImagesLabel')}</label>
          <p style={{ fontSize: '0.75rem', color: '#A07840', marginBottom: '0.75rem' }}>{t('imagesHint')}</p>
          ...
          <button type="button" onClick={() => imgInputRef.current?.click()} disabled={uploading} style={btnStyle(uploading ? '#A07840' : '#1B2E4A')}>
            {uploading ? t('uploadingLabel') : t('uploadImagesBtn')}
          </button>
        </div>

        {/* Videos */}
        <div style={{ background: '#FFF5E0', border: '1.5px solid #DDB840', borderRadius: 8, padding: '1.25rem' }}>
          <label style={labelStyle}>🎥 {t('addVideoLabel')}</label>
          ...
          <div style={{ marginBottom: '0.75rem' }}>
            ...
            <button type="button" onClick={() => vidInputRef.current?.click()} disabled={uploadingVideo} style={{ ...btnStyle(uploadingVideo ? '#A07840' : '#E8380A'), marginBottom: 6, display: 'block' }}>
              {uploadingVideo ? t('uploadingLabel') : `⬆ ${t('uploadVideoBtn')}`}
            </button>
            <div style={{ fontSize: '0.7rem', color: '#A07840' }}>{t('videoHint')}</div>
          </div>

          {/* YouTube URL */}
          <div style={{ borderTop: '1px solid #DDB840', paddingTop: '0.75rem' }}>
            <div style={{ fontSize: '0.7rem', fontWeight: 700, color: '#6B4820', marginBottom: 5 }}>{t('orPasteYoutube')}</div>
            <div style={{ display: 'flex', gap: 6 }}>
              ...
              />
              <button type="button" onClick={addYouTube} style={btnStyle('#D4A000')}>{t('addBtn')}</button>
            </div>
